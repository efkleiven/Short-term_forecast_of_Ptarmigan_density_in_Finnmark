...
BugsData$T <- dim(BugsData$y)[2]-3 # include only data until 2021
...
# run model in JAGS
out_early_21 <- jags(BugsData, inits, params, "earlymodel.txt", n.thin=nt,
                 n.chains=nc, n.burnin=nb, n.adapt=na, n.iter=ni, parallel=T)   # 

# keep only MCMC values
out_early_21 <- out_early_21$sims.list

# save output
save(out_early_21, file="output/early_00-21.RData")
