...
"model{
  ...
  for(s in 1:nsites){  # sites/lines 
    ...
    for (t in 2:T){
      ...
      mu[s,t] <- bt0[Reg[s]] + rCl[Clust[s]]+ 
      #btDD * mu[s,t-1] + btR * rod[t, Reg[s]] + btRD * rod[t-1, Reg[s]] + 
...
#read data
load("data/RypeData_GBIF00-23_v4.rds")
...
params <- c("sdproctau","sdprectau","PrOc","PrEc", "alpha0","btf0", "bt0", "bt1", "rCl",
            "btDoYf", "btDoYft","btDoYfs", "bttrend",
            "Ntotal", "mu", "D", "Dtot")   
...
out23 <- jags(BugsData, inits, params, "climatemodel.txt", n.thin=nt,
                 n.chains=nc, n.burnin=nb, n.adapt=na, n.iter=ni, parallel=T)   # 

setwd("./output")
save(out23, file="climatemodel_00-23.RData")
#- end of script
