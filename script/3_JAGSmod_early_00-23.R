...
BugsData$T <- dim(BugsData$y)[2]-1 # include only data until 2023
...
# run model in JAGS
out_early_23 <- jags(BugsData, inits, params, "earlymodel.txt", n.thin=nt,
                 n.chains=nc, n.burnin=nb, n.adapt=na, n.iter=ni, parallel=T)   # 

# keep only MCMC values
out_early_23 <- out_early_23$sims.list

# save output
save(out_early_23, file="output/early_00-23.RData")
