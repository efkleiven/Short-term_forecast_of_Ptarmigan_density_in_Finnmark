...
BugsData$T <- dim(BugsData$y)[2]-2  # to get data until 2022
...
# run model in JAGS
out_full_22 <- jags(BugsData, inits, params, "earlymodel.txt", n.thin=nt,
                 n.chains=nc, n.burnin=nb, n.adapt=na, n.iter=ni, parallel=T)   # 

# keep only MCMC values
out_full_22 <- out_full_22$sims.list

# save output
save(out_full_22, file="output/fullmod_00-22.RData")
