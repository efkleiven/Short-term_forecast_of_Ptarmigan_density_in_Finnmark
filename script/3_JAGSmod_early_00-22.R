...
BugsData$T <- dim(BugsData$y)[2]-2 # include only data until 2022
...
# run model in JAGS
out_early_22 <- jags(BugsData, inits, params, "earlymodel.txt", n.thin=nt,
                 n.chains=nc, n.burnin=nb, n.adapt=na, n.iter=ni, parallel=T)   # 

# keep only MCMC values
out_early_22 <- out_early_22$sims.list

# save output
save(out_early_22, file="output/early_00-22.RData")
