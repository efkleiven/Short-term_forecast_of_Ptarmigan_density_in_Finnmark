...
d_trans3 <- d_trans %>%
  ...
  ungroup()%>%
  filter(!locality %in% c("Sørøya", "Rolvsøy", "Hjelmsøy"))

length(unique(d_trans2$locationID))
unique(d_trans2$locality)
...
# add column for site
sitename <- unique(d_obs3$locationID)
clustname <- unique(d_obs3$locality)
regname <- unique(d_obs3$verbatimLocality)
...
save(d_trans3, file="Short-term_forecast_of_Ptarmigan_density_in_Finnmark/data/d_trans_3.rds")

# Save d_obs for summary of observations
save(d_obs3, file="Short-term_forecast_of_Ptarmigan_density_in_Finnmark/data/d_obs_3.rds")
#------------------------------------------------------------------------------------------------------------
